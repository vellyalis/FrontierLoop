...
description: "独自Engine・Framework・Runtime・Protocol・Algorithmなど成熟した直接解がない実装を、問題モデル・参照Oracle・Vertical Slice・実験Ledger・性能予算で収束させる。"
---
...
焦点化した局所変更または一回の可逆実験で判断できる場合は、まず`routine-change`または既存`architecture`を使用する。起動時に、現在の一つのMaterial Decision、Current Bottleneck、Skillを終了できる観測を明示する。
...
- Prototype、Benchmark Harness、Debug InstrumentationをProduction Ownerへ昇格させない。
