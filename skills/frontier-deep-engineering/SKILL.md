...
description: "Explicit-only: resolve a hard mechanism, algorithm, or premise that simpler approaches and focused experiments cannot settle. Not difficulty, novelty, or framework work alone."
---
...
焦点化した局所変更または一回の可逆実験で判断できる場合は、`frontier-core`で進め、Materialな責任境界の判断だけ既存`frontier-architecture`へ渡す。起動時に、現在の一つのMaterial Decision、Current Bottleneck、Skillを終了できる観測を明示する。
...
### 1A. First-principles reconstruction

Only after the first-principles lane gate above is satisfied, read
[the reconstruction method](references/first-principles.md). Do not load it for
an ordinary hard implementation whose mechanism and evaluator are already clear.
...
For each material experiment, record only decision-bearing evidence in the existing project record:
